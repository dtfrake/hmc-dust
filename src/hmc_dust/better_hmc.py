...
def sample_hmc(
    ...
    def sampling_scan_body(carry, _):
        step, position, momentum, current_rng = carry
        position, momentum, exact_positions, accept_prob, next_rng = one_overall_step(position, momentum, current_rng)
        step = step + 1
        jax.lax.cond(
            step % 5 == 0,
            lambda: jax.debug.print("HMC sample {s}/{n}", s=step, n=num_samples),
            lambda: None,
        )
        return (step, position, momentum, next_rng), (position, momentum, exact_positions, accept_prob)
 
    start_momentum = jnp.zeros_like(start_position)
    (_, _, _, final_rng), (positions, momenta, overall_exact_position_arr, accept_probs) = jax.lax.scan(
        sampling_scan_body, (jnp.int32(0), start_position, start_momentum, rng), None, length=num_samples
    )
