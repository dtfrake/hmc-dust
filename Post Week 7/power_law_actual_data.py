...
fourier_length = dx*num_fourier_dimensions
...
inv_mass_matrix = jnp.eye(num_fourier_dimensions + num_fitting_params + num_data + 1)
...
ending_indices = jnp.arange(num_fourier_dimensions + num_fitting_params + 1,
                            num_fourier_dimensions + num_fitting_params + 1 + num_data)
...
def power_law_field(xi, inference_params):
    log_overall_variance, log_exponent, log_zero_mean, log_zero_logvar, offset = inference_params
    exponent = -1*jnp.exp(log_exponent)
    log_zero_std = jnp.exp(0.5*log_zero_logvar)       

    log_first_power = log_zero_mean + xi[0]*log_zero_std

    omegas = 2*jnp.pi*jnp.fft.fftfreq(num_fourier_dimensions, d=dx)[1:]
    ...
    return field[:num_dimensions] + offset

def response_function(xi, r, inference_params):
    field = power_law_field(xi, inference_params) 
...
def log_params_prior(inference_params):
    log_overall_variance, log_exponent, log_zero_mean, log_zero_var, offset = inference_params 
    return -0.5*log_overall_variance**2 - 0.5*log_exponent**2 - 0.5*log_zero_mean**2 - 0.5*log_zero_var**2 - 0.5*offset**2


def negative_logdensity(x):
    n_xi = num_fourier_dimensions + 1
    inference_params = x[:num_fitting_params]
    xi = x[num_fitting_params : num_fitting_params + n_xi]
    r  = x[num_fitting_params + n_xi :]
    # There are priors on everything: logvar, lognu, offset, xi, and r
    negative_log_p_s = -log_params_prior(inference_params) + 0.5*xi.T @ xi - jnp.sum(distance_logprior(r))
    field_res = y_obs - response_function(xi, r, inference_params)
...
offsets = overall_data_arr[:, 4]
overall_extinction_arr    = overall_data_arr[:, num_fitting_params : num_fitting_params + num_fourier_dimensions + 1]
overall_star_location_arr = overall_data_arr[:, num_fitting_params + num_fourier_dimensions + 1 :]

mean_star_location_arr = overall_star_location_arr.mean(axis=0)

inference_params = (log_overall_variances, log_exponents, log_zero_means, log_zero_logvars, offsets)

overall_extinction_arr_raw = jax.vmap(power_law_field, in_axes=(0, 0))(
...
timegrid = jnp.arange(num_fourier_dimensions + num_fitting_params + num_data + 1)
...
axes[1][1].plot(time_arr, log_overall_variances, color = "Orange")
axes[1][1].set_title("Log Variance Trace Plot")
...
axes[2][2].plot(time_arr, log_zero_means, color = "Orange")
axes[2][2].set_title("Trace Plot of Log Zero Means")
...
for i in range (0, 3):
    ...
    for j in range (0, 3):
        ...
        log_r_posterior = logposterior_on_r(star_linspace, plx_obs[star_indices[i + 3*j]], plx_err[mask][star_indices[i + 3*j]])
