...
INV_GAMMA_MEAN = float(os.environ.get("INV_GAMMA_MEAN", 4))
INV_GAMMA_VAR  = float(os.environ.get("INV_GAMMA_VAR", 16))


from hmc_dust import DATA, GAIA

RUN = os.environ.get("SLURM_JOB_ID", "local")
TAG = f"M{INV_GAMMA_MEAN:g}_V{INV_GAMMA_VAR:g}_{RUN}"
OUTDIR = os.environ.get("HMC_DUST_OUT", "runs")
os.makedirs(OUTDIR, exist_ok=True)

def out(name):
    root, ext = os.path.splitext(name)
    return os.path.join(OUTDIR, f"{root}_{TAG}{ext}")
...
print(f"[config] inv-gamma mean={INV_GAMMA_MEAN} var={INV_GAMMA_VAR} -> "
      f"alpha={inv_gamma_variance_prior_alpha:.4f} beta={inv_gamma_variance_prior_beta:.4f} "
      f"nu={student_t_nu:.4f} sigma={student_t_sigma:.4f}", flush=True)
