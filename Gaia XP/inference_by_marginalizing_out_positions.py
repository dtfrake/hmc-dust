...
x_error = jax.vmap(get_x_err_basic)(data_plx_err, plx_obs)
num_data = x_obs.size
inv_cov_data_matrix = jnp.linalg.inv(jnp.diag(data_ext_err**2))



fig, axes = plt.subplots(1, 3, figsize = (18, 5))
axes[2].errorbar(x_obs, y_obs, yerr = data_ext_err, xerr = x_error, fmt='.', markersize=4, ecolor='#9ab8d8', elinewidth=0.8, capsize=0, alpha=0.8, label='observed')
...
axes[0].errorbar(distances_psc[mask], ext[mask],
             yerr=data_ext_err,      # vertical error bars
...
burn_in = 50
num_integration_steps = 6000
initial_step_size = 0.0002
...
#%%
def logparamsprior(inference_params, offset):
    logvar, lognu = inference_params
    return -0.5*(logvar - logvar_prior_mean)**2 -0.5*(lognu - lognu_prior_mean)**2 -0.5*(offset - offset_prior_mean)**2

# This returns the integrated denstity at dust_x_linspace. It does NOT do so at star locations, because we are not inferring
# star locations! Instead, we must integrate over all possible locations to get the likelihood for each star/data point.
def response_function(xi, inference_params, offset):
    field = fft_field_hartley(xi, inference_params) + offset
    return exponentiated_integrated_density(field)
...
def star_loglike_integrated_over_r(plx_obs, plx_err, ext_obs, ext_err, field):
    loglikes = exact_position_loglike(jnp.arange(total_integrating_dims),
                                      plx_obs, plx_err, ext_obs, ext_err, field)
...
def negative_logdensity(x):
    ...
    negative_log_p_s = -logparamsprior(inference_params, offset) + 0.5*xi.T @ xi 
    field = response_function(xi, inference_params, offset)
    star_integrated_log_likelihoods = jax.vmap(star_loglike_integrated_over_r, in_axes=(0, 0, 0, 0, None))(plx_obs, data_plx_err, y_obs, data_ext_err, field)
    negative_log_p_d_given_s = -jnp.sum(star_integrated_log_likelihoods)
    return negative_log_p_s + negative_log_p_d_given_s

if(doing_burn_in):
    initial_coordinates = jnp.zeros(num_fourier_dimensions + num_fitting_params) 
    initial_coordinates = initial_coordinates.at[:3].set(jnp.array([initial_logvar, initial_lognu, initial_offset]))
else:
    initial_coordinates = jnp.asarray(np.load("last_position_Gaia_XP_fitting_actual_data.npy"))

# Size of initial coordinates: num_fourier_dimensions + num_fitting_params
...
np.save(out('overall_data_integrating_positions.npy'), overall_data_arr)
np.save(out('accept_prob_integrating_positions.npy'), accept_prob_arr)

#%%
logvars, lognus, offsets = overall_data_arr[:, :3].T
...
overall_extinction_arr_raw = jax.vmap(fft_field_hartley, in_axes=(0, 0))(overall_extinction_arr, inference_params) + offsets[:, None]
overall_extinction_arr_exp = jax.vmap(exponentiated_integrated_density, in_axes=(0))(overall_extinction_arr_raw)
...
#inner = jax.vmap(jax.vmap(exact_position_loglike, in_axes=(None, 0, 0, 0, 0, None)),
                 #in_axes=(0, None, None, None, None, None))
#idxs = jnp.arange(total_integrating_dims)
#log_sum, _ = jax.lax.scan(
    #lambda acc, field: (jnp.logaddexp(acc, inner(idxs, plx_obs, data_plx_err, y_obs, data_ext_err, field)), None),
    #jnp.full((total_integrating_dims, plx_obs.size), -jnp.inf),
    #overall_extinction_arr)
#log_mean = log_sum - jnp.log(overall_extinction_arr.shape[0])


loglike_grid = jax.vmap(                                  # -> axis 0: positions
    jax.vmap(                                             # -> axis 1: stars
        jax.vmap(exact_position_loglike,                  # -> axis 2: samples
                 in_axes=(None, None, None, None, None, 0)),
        in_axes=(None, 0, 0, 0, 0, None)),
    in_axes=(0, None, None, None, None, None))


star_posterior_array = loglike_grid(jnp.arange(total_integrating_dims), plx_obs, data_plx_err, y_obs, data_ext_err,
                      overall_extinction_arr_exp)   # [P, NUM_STARS, NUM_SAMPLES]
log_mean = logsumexp(star_posterior_array, axis=2)
...
fig.savefig(out("covariance_plots_integrated_position.png"), dpi=120, bbox_inches="tight")


#%%
fig, axes = plt.subplots(8, 3, figsize = (24, 40))
star_indices = jnp.arange(num_data)
star_mask = (mean_star_location_arr_analytic < 1200) & (data_ext_err < 0.01)
...
axes[0][2].scatter(mean_star_location_arr_analytic, y_obs, color = "purple", s = 2, zorder = 10, label = "Mean HMC star locations")
axes[0][2].legend()
...
indices_plot_10 = np.array([30*i for i in range(10)])
...
for i in range (0, 3):
    axes[3][i].plot(dust_x_linspace, overall_extinction_arr_exp[samples_to_highlight[i], :], color = 'orange', alpha = 0.7, zorder = 10, label = "HMC sample extinction")
    axes[3][i].plot(dust_x_linspace, jnp.interp(dust_x_linspace, radial_boundaries, cumulative_integrated_dust), color = "red", alpha = 0.7, zorder = 20, label = "True Edenhofer integrated extinction")
    axes[3][i].errorbar(x_obs, y_obs, yerr = data_ext_err, xerr = x_error, fmt='.', markersize=4, ecolor='#9ab8d8', elinewidth=0.8, capsize=0, alpha=0.8, zorder = 1, label = "Extinction/location data errorbars")
    axes[3][i].set_title(f"HMC sample {samples_to_highlight[i]}")
    axes[3][i].legend()
    max_v = float(jnp.max(mean_extinction_exp_int))
    for j in range (0, 3):
        ax = axes[5 + j][i]
        ax.plot(integrating_dust_x_linspace, star_posterior_2d[:, star_indices[i + 3*j]], color = "black", label = "Analytical position posterior")
        ...
        log_r_posterior = logposterior_on_r(star_linspace, plx_obs[star_indices[i + 3*j]], data_plx_err[star_indices[i + 3*j]])
        ...
        ax.set_title(f"x_obs = {x_obs[star_indices[i + 3*j]]:.3f} plx_err = {data_plx_err[star_indices[i + 3*j]]:.3f}, ext_obs = {y_obs[star_indices[i + 3*j]]:.3f} ext_err = {data_ext_err[star_indices[i + 3*j]]:.3f}")
...
axes[4][2].plot(time_arr, offsets, color = "Purple")
axes[4][2].set_title("Offset Trace Plot")


fig.tight_layout()


fig.savefig(out("final_plots_integrated_position.png"), dpi=120, bbox_inches="tight")
...
indices_of_interest = {"Variance": 0, "Nu": 1, "29th": 31}
