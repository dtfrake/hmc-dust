...
initial_lognu = 0
initial_offset = -7.6
initial_logistic_bernoulli = -4
logvar_prior_mean = initial_logvar
lognu_prior_mean = 0.0
offset_prior_mean = initial_offset
logistic_bernoulli_prior_mean = initial_logistic_bernoulli
...
def logbernoulliprior(logisticbernoulli):
    return -0.5*(logisticbernoulli - logistic_bernoulli_prior_mean)**2

def negative_logdensity(x):
    logvar = x[0]
    lognu = x[1]
    offset = x[2]
    logistic_bernoulli = x[3]
    bernoulli = 1/(1 + jnp.exp(-1*logistic_bernoulli))
    ...
    log_data_likelihoods = jnp.logaddexp(log_data_likelihoods, np.log(bernoulli))
...
if(doing_burn_in):
    ...
    initial_coordiantes = initial_corodaintes.at[3].set(initial_logistic_bernoulli)
...
HERE = Path(__file__).resolve().parent          # .../hmc-dust/Gaia XP
SHERLOCK = HERE.parent / "sherlock_results"     # .../hmc-dust/sherlock_results

overall_data_arr = jnp.asarray(
    np.load(SHERLOCK / "overall_data_bernoulli_outliers_40060023.npy")
)
...
logistic_bernoullis = overall_data_arr[:, 3]
...
nus = jnp.exp(lognus)
bernoullis = 1/(1 + jnp.exp(-1*logistic_bernoullis))
...
fig.tight_layout()
fig.savefig(out("covariance_plots_bernoulli_outliers.png"), dpi=120, bbox_inches="tight")
#%%
...
axes[3][2].plot(time_arr, logistic_bernoullis, color = "Orange")
axes[3][2].set_title("Trace Plot of Logistic Bernoullis")
...
fig.tight_layout()
fig.savefig(out("final_plots_bernoulli_outliers.png"), dpi=120, bbox_inches="tight")
